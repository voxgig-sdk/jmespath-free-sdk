...
class JmesPathEntity
  ...
  def make
    opts = @_entopts.dup
    JmesPathEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = JmespathFreeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [JmesPath, Hash] the current JmesPath data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = JmespathFreeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of JmesPath fields)
  def match_get
  ...
  #
  # @param reqdata [JmesPathCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [JmesPath, Hash] the created JmesPath; raises JmespathFreeError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = JmespathFreeHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end

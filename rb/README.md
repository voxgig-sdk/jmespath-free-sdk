# JmespathFree Ruby SDK



The Ruby SDK for the JmespathFree API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.JmesPath` — with named operations (`create`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = JmespathFreeSDK.test(testopts, sdkopts)
```
...
raise a `JmespathFreeError` (a `StandardError` subclass), so wrap
...
```
rb/
├── JmespathFree_sdk.rb       -- Main SDK module
...
The main module (`JmespathFree_sdk`) exports the SDK class

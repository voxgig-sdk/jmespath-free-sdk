# JmespathFree SDK test runner

from __future__ import annotations
import os
import json

from jmespathfree_sdk.utility.voxgig_struct import voxgig_struct as vs


class JmespathFreeTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            JmespathFreeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = JmespathFreeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = JmespathFreeTestRunner.getenv("JMESPATH_FREE_TEST_LIVE")
        override = JmespathFreeTestRunner.getenv("JMESPATH_FREE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = JmespathFreeTestRunner.getenv(key)
        ...
        explain = JmespathFreeTestRunner.getenv("JMESPATH_FREE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["JMESPATH_FREE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if JmespathFreeTestRunner._test_control is not None:
            return JmespathFreeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                JmespathFreeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            JmespathFreeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = JmespathFreeTestRunner.load_test_control()
...
def load_env_local():
    JmespathFreeTestRunner.load_env_local()


def env_override(m):
    return JmespathFreeTestRunner.env_override(m)


def entity_data(v):
    return JmespathFreeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return JmespathFreeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return JmespathFreeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return JmespathFreeTestRunner.load_test_control()


def live_client_options():
    return JmespathFreeTestRunner.live_client_options()


def live_delay_ms():
    return JmespathFreeTestRunner.live_delay_ms()

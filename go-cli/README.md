...
boru-driven command-line client **and** interactive REPL for the JmespathFree
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/jmespath-free-cli)
make build

# 2. See usage (words, entities, env vars)
./jmespath-free-cli --help
...
JMESPATH_FREE_BASE=https://api.example.com ./jmespath-free-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/jmespath-free-cli
   ```
...
Run with no arguments to open a REPL (prompt `jmespath-free>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `jmes_path`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/jmespath-free-cli`. |
...
(`./jmespath-free-cli <expr>`) and interactively (the REPL), and expressions compose the

...
describe("JmesPathEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set JMESPATH_FREE_TEST_JMES_PATH_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    jmes_path_ref01_data = helpers.to_map(type(jmes_path_ref01_data_result) == 'table' and jmes_path_ref01_data_result.data_get and jmes_path_ref01_data_result:data_get() or jmes_path_ref01_data_result)
...
function jmes_path_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("JMESPATH_FREE_TEST_JMES_PATH_ENTID")
  ...
  if env["JMESPATH_FREE_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["JMESPATH_FREE_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,

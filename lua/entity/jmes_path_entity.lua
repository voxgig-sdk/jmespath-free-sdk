...
local JmesPathEntity = {}
JmesPathEntity.__index = JmesPathEntity


function JmesPathEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, JmesPathEntity)
  self._name = "jmes_path"
  self._client = client
...
function JmesPathEntity:get_name()
  return self._name
end


function JmesPathEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return JmesPathEntity.new(self._client, opts)
end
...
function JmesPathEntity:mark_deleted()
  self._deleted = true
end


function JmesPathEntity:deleted()
  return true == self._deleted
end


function JmesPathEntity:data_set(args)
  if args ~= nil then
...
function JmesPathEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function JmesPathEntity:match_set(args)
  if args ~= nil then
...
function JmesPathEntity:match_get()
...
function JmesPathEntity:stream(action, args, callopts)
...
function JmesPathEntity:_run_op(ctx, post_done)
  local utility = self._utility

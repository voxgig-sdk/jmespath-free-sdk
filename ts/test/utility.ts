...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.JMESPATHFREE_TEST_LIVE ||
    'TRUE' === process.env.JMESPATHFREE_TEST_OVERRIDE
  ) {
  ...
  m.JMESPATHFREE_TEST_EXPLAIN = process.env.JMESPATHFREE_TEST_EXPLAIN || m.JMESPATHFREE_TEST_EXPLAIN

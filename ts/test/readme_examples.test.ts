...
//     (\`new JmespathFreeSDK(...)\` / \`JmespathFreeSDK.test(...)\` become
//     \`JmespathFreeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'JmespathFreeSDK'
...
const TEST_SEED = {"entity":{"jmes_path":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new JmespathFreeSDK(...)\` and any
// \`JmespathFreeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+JmespathFreeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/JmespathFreeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

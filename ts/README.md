# JmespathFree TypeScript SDK



The TypeScript SDK for the JmespathFree API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.JmesPath()` — each with a small set of operations (`create`)
...
```ts
try {
  const jmespath = await client.JmesPath().create({ data: "example", query: "example" })
...
```ts
const entity = client.JmesPath()

// First call runs the operation and stores its result
await entity.create({ data: 'example_data', query: 'example_query' })
...
| `tester(testopts?, sdkopts?)` | `JmespathFreeSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `JmespathFreeSDK.test(testopts?, sdkopts?)` | `JmespathFreeSDK` | Create a test-mode client. |
...
| `client` | `client(): JmespathFreeSDK` | Return the parent SDK client. |
...
jmespath-free/
├── src/
│   ├── JmespathFreeSDK.ts        # Main SDK class
...
```ts
const jmespath = client.JmesPath()
await jmespath.create({ data: "example", query: "example" })

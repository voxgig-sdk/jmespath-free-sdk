...
module JmespathFreeUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = JmespathFreeResponse.new({})
    ...
    if fetch_err
      ...
      response = JmespathFreeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = JmespathFreeResponse.new(fetched)
    else

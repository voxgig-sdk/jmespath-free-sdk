...
class JmespathFreeMakeError
{
    public static function call(?JmespathFreeContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new JmespathFreeContext([], null);
        }
        $op = $ctx->op ?? new JmespathFreeOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new JmespathFreeResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof JmespathFreeError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new JmespathFreeError('', $msg, $ctx);
        ...
        if ($err instanceof JmespathFreeError) {

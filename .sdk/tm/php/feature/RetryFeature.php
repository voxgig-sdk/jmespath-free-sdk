...
// JmespathFree SDK retry feature
...
class JmespathFreeRetryFeature extends JmespathFreeBaseFeature
{
    ...
    public function init(JmespathFreeContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (JmespathFreeContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(JmespathFreeContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(JmespathFreeContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;

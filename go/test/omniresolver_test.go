...
import (
	...
	omni "github.com/voxgig-sdk/jmespath-free-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.JmespathFreeError); is {
				out := map[string]any{
					"name":    "JmespathFreeError",
...
func omniCtx(arg any, client *sdk.JmespathFreeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.JmespathFreeSDK); is {
			client = live

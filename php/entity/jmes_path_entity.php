...
// JmespathFree SDK JmesPath entity
...
class JmesPathEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new JmesPathEntity($this->_client, $opts);
    }

    /**
     * @param JmesPath|array $args JmesPath data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = JmespathFreeHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return JmesPath|array The current JmesPath data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of JmesPath fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = JmespathFreeHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of JmesPath fields).
     */
    ...
    /**
     * Create a new JmesPath.
     *
     * @param JmesPathCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed JmesPathCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return JmesPath|array The created JmesPath as an assoc-array at the
     *   SDK boundary; throws JmespathFreeError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = JmespathFreeHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }

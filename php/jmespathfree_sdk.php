...
class JmespathFreeSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = JmespathFreeHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = JmespathFreeHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!JmespathFreeFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, JmespathFreeFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return JmespathFreeUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = JmespathFreeHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = JmespathFreeHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = JmespathFreeHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new JmespathFreeError($op . "_allow",
                "JmespathFreeSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = JmespathFreeHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new JmespathFreeError("graphql_error",
                "JmespathFreeSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function JmesPath($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_jmes_path === null) {
                $this->_jmes_path = new JmesPathEntity($this, null);
            }
            return $this->_jmes_path;
        }
        return new JmesPathEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new JmespathFreeSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}

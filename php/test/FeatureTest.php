...
class FtCtrl extends JmespathFreeControl
{
...
class FtClient
{
    ...
    private JmespathFreeUtility $utility;

    public function __construct(JmespathFreeUtility $utility, array $options)
    {
    ...
    public function get_utility(): JmespathFreeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (JmespathFreeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public JmespathFreeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (JmespathFreeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = JmespathFreeConfig::make_feature($name);
    ...
    private static function buildUrl(JmespathFreeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof JmespathFreeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new JmespathFreeResponse($fetched);
        }
    ...
    private function fail(JmespathFreeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (JmespathFreeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

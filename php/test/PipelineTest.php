...
// JmespathFree SDK pipeline test
//
...
{
    private static function utility(): JmespathFreeUtility
    {
        return new JmespathFreeUtility();
    }
    ...
    private static function ctx(array $over = []): JmespathFreeContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new JmespathFreeContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new JmespathFreeOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof JmespathFreeError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new JmespathFreeOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = JmespathFreeMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new JmespathFreeOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = JmespathFreeMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new JmespathFreeOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = JmespathFreeMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new JmespathFreeSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = JmespathFreeMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new JmespathFreeResponse([]),
            'result' => new JmespathFreeResult([]),
        ]);
        [, $err] = JmespathFreeMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new JmespathFreeSpec([]),
            'response' => null,
            'result' => new JmespathFreeResult([]),
        ]);
        [, $err] = JmespathFreeMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new JmespathFreeSpec(['step' => 's']),
            'response' => new JmespathFreeResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new JmespathFreeResult([]),
        ]);
        [, $err] = JmespathFreeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new JmespathFreeSpec(['step' => 's']),
            'response' => new JmespathFreeResponse(self::resp(200, ['v' => 1])),
            'result' => new JmespathFreeResult([]),
        ]);
        [, $err] = JmespathFreeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new JmespathFreeSpec(['step' => 's']),
            'response' => new JmespathFreeResponse(self::resp(200, ['v' => 2])),
            'result' => new JmespathFreeResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        JmespathFreeMakeResponse::call($ctx);
    ...
    {
        $preset = new JmespathFreeResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new JmespathFreeSpec([]),
            'response' => new JmespathFreeResponse([]),
            'result' => new JmespathFreeResult([]),
        ...
        [$got, $err] = JmespathFreeMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new JmespathFreeResult([])]);
        [, $err] = JmespathFreeMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new JmespathFreeSpec([]), 'result' => null]);
        [, $err] = JmespathFreeMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new JmespathFreeSpec(['step' => 's']),
            'result' => new JmespathFreeResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new JmespathFreeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = JmespathFreeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new JmespathFreeSpec(['step' => 's']),
            'result' => new JmespathFreeResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new JmespathFreeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = JmespathFreeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new JmespathFreeResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new JmespathFreeSpec([]),
            'result' => new JmespathFreeResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = JmespathFreeMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = JmespathFreeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new JmespathFreeError('boom', 'boom');
        $utility->fetcher = function (JmespathFreeContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new JmespathFreeSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = JmespathFreeMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (JmespathFreeContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = JmespathFreeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(JmespathFreeResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        JmespathFreeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (JmespathFreeContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new JmespathFreeSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = JmespathFreeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new JmespathFreeResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new JmespathFreeSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = JmespathFreeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = JmespathFreeMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = JmespathFreeMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new JmespathFreeResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, JmespathFreeDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new JmespathFreeResult(['ok' => false])]);
        $this->expectException(JmespathFreeError::class);
        JmespathFreeDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new JmespathFreeResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', JmespathFreeMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        JmespathFreeMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new JmespathFreeBaseFeature();
        $b = new JmespathFreeBaseFeature();
        JmespathFreeFeatureAdd::call($ctx, $a);
        JmespathFreeFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): JmespathFreeBaseFeature
    {
        $f = new JmespathFreeBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        JmespathFreeFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): JmespathFreeContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new JmespathFreeSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = JmespathFreePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        JmespathFreePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new JmespathFreeResponse(['headers' => null]),
            'result' => new JmespathFreeResult([]),
        ]);
        JmespathFreeResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        JmespathFreeResultBody::call($ctx);
